...
class SiteFrame(ttk.TTkFrame):
  def __init__(self,parent, site:Site, *args, **kwargs):
    ttk.TTkFrame.__init__(self,title=site.name, *args, **kwargs)
    self.parent:HttpCheckApp = parent
    self.site = site
    ...
    self.editbutton.clicked.connect(lambda: self.editSite())
  ...
  def deleteSite(self):
    self.parent.deleteSite(self.site)
  def editSite(self):
    editWindow = ttk.TTkWindow(pos = (3,3), size=(30,10), title="Edit")
    ttk.TTkLabel(parent=editWindow, pos=(5,5), text="Hello World")
...
class HttpCheckApp:
  def __init__(self, master: ttk.TTk) -> None:
    ...
    for site in self.siteList:
      self.mainLayout.addWidget(SiteFrame(self, site), currow, curcol)
      curcol += 1
      if curcol == 6:
        currow += 1
        curcol = 2
  ...
  def deleteSite(self, site: Site):
    ...
    self.mainLayout.removeWidget(site.frame)
    self.saveConfig()
